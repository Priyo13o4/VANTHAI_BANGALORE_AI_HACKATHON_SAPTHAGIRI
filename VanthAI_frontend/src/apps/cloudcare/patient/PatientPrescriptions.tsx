...
import { MOCK_PRESCRIPTIONS, MOCK_PRESCRIPTION_DETAILS } from '../data/mockData';
...
export default function PatientPrescriptions() {
  ...
  const getActivePrescriptions = () => {
    return (MOCK_PRESCRIPTIONS as any[]).filter(isActivePrescription);
  };

  const getPastPrescriptions = () => {
    return (MOCK_PRESCRIPTIONS as any[]).filter((p) => !isActivePrescription(p));
  };

  const getDisplayedPrescriptions = () => {
    switch (activeTab) {
      case 0: // All
        return MOCK_PRESCRIPTIONS as any[];
      case 1: // Active
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {displayedPrescriptions.map((prescription) => {
                        const details = (MOCK_PRESCRIPTION_DETAILS as any)[prescription.id];
