...
export default function DoctorLogin() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<LoginFormData>({
    ...
    defaultValues: { email: 'sarah.johnson@hospital.com', password: 'test123' },
  });
      ...
      <Box sx={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'center', py: 4 }}>
        <Paper elevation={3} sx={{ p: { xs: 3, sm: 5 }, borderRadius: 3 }}>
          <Box sx={{ textAlign: 'center', mb: 4 }}>
            <HealingIcon sx={{ fontSize: 60, color: 'secondary.main', mb: 2 }} />
            <Typography variant="h4" gutterBottom sx={{ fontWeight: 600 }}>Doctor Portal</Typography>
            <Typography variant="body2" color="text.secondary">Secure access for healthcare professionals</Typography>
          </Box>
          {error && <Alert severity="error" sx={{ mb: 3 }}>{error}</Alert>}
          <Alert severity="info" sx={{ mb: 3 }}>
            <Typography variant="body2" sx={{ fontWeight: 500 }}>Demo Credentials:</Typography>
            <Typography variant="caption">Email: sarah.johnson@hospital.com | Password: test123</Typography>
          </Alert>
          <form onSubmit={handleSubmit(onSubmit)}>
            <TextField fullWidth label="Doctor Email" {...register('email')} error={!!errors.email} helperText={errors.email?.message} slotProps={{ input: { startAdornment: <InputAdornment position="start"><EmailIcon color="action" /></InputAdornment> } }} sx={{ mb: 3 }} disabled={loading} />
            ...
            <Button type="submit" variant="contained" color="secondary" size="large" fullWidth disabled={loading} sx={{ mb: 2, py: 1.5 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, mt: 2 }}>
            ...
            <Button variant="outlined" fullWidth onClick={() => navigate('/cloudcare/hospital-login')} disabled={loading}>Hospital Login</Button>
          </Box>
        </Paper>
      </Box>
    </Container>
